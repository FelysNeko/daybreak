...
pub fn memoize_helper(meta: TokenStream, body: TokenStream) -> TokenStream {
    ...
    let fast = quote! {
        ...
        let _m_mode = self.stream.mode();
        let _m_cache_type = Self::CT::#cache #args;
        if let Some(_m_cache) = self.cache.get(_m_pos, _m_mode, _m_cache_type) {
    ...
    let store = quote! {
        ...
        let _m_mode = self.stream.mode();
        self.cache.insert(_m_pos, _m_mode, _m_cache_type, _m_end, _m_cache_result);
        _m_result
    };
...
pub fn lecursion_helper(meta: TokenStream, body: TokenStream) -> TokenStream {
    ...
    let main = quote! {
        ...
        loop {
            let _l_mode = self.stream.mode();
            self.cache.insert(_l_pos, _l_mode, _l_cache_type, _l_end, _l_cache_result.clone());
            let _l_res = || #rt #block();
...
pub fn strict_helper(_: TokenStream, body: TokenStream) -> TokenStream {
    ...
    quote!(
        #(#attrs)*
        #vis #signature {
            let _s_mode = self.stream.mode();
            let _s_res = || #rt #block();
            self.stream.strict(_s_mode);
            _s_res
        }
    ).into()
}
